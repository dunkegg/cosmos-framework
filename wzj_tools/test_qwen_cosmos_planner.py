...
CANDIDATE_OUTPUT_DIR = "outputs/candidate_reward_test"
SLIGHT_TURN_YAW_DEG = 8.0
...
def qwen_chat(messages: list[dict[str, Any]], temperature: float = 0.1, max_tokens: int = 256) -> str:
...
Judge whether the predicted rollout makes meaningful progress toward the
instruction compared with the current observation.

Evaluate:
- subgoal_progress: 0..1
- direction_correctness: 0..1
- target_visibility_gain: 0..1
...
def qwen_progress_reward(
    ...
    factor_reward = (
        ...
        + 0.15 * score["target_visibility_gain"]
        + 0.20 * score["route_consistency"]
        - 0.50 * score["collision_risk"]
        - 0.30 * score["regression"]
    )

    score["factor_reward"] = float(factor_reward)
    score["confidence_weighted_reward"] = float(factor_reward * score["confidence"])
...
def run_three_candidate_reward_test(
    ...
    candidates = make_test_candidates(distance_m=1.2)
    ...
    for candidate in candidates:
        ...
        selection_reward = float(reward["confidence_weighted_reward"])

        result = {
            "name": name,
            "plan": plan,
            "selection_reward": selection_reward,
            "progress": float(reward["progress"]),
            "confidence": float(reward["confidence"]),
            "subgoal_progress": float(reward["subgoal_progress"]),
            "direction_correctness": float(reward["direction_correctness"]),
            "route_consistency": float(reward["route_consistency"]),
            "collision_risk": float(reward["collision_risk"]),
            "regression": float(reward["regression"]),
    ...
    summary = {
        "instruction": instruction,
        "selection_metric": "confidence_weighted_reward",
    ...
    for rank_idx, item in enumerate(ranking, start=1):
        print(
            ...
            f"reward={item['selection_reward']:+.4f} | "
            f"progress={item['progress']:+.3f} | "
            f"direction={item['direction_correctness']:.3f} | "
